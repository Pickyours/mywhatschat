...
class WebhookController extends BaseController
{
    ...
    {
        // Get JSON data from CRM webhook
        $data = $request->all();
        // Log received data (for debugging)
        Log::info('Webhook Received:', $data);
        // Fetch Template from WhatsApp API
        $templateResponse = Http::get('https://mywhatschat.in/api/templates/' . $data['uuid']);
        $template = $templateResponse->json();
        if (!$template || !isset($template['metadata']['components'])) {
        return response()->json(['error' => 'Template not found or invalid'], 404);
        }
        
        // Extract WhatsApp Template Data
        $templateBody = null;
        foreach ($template['metadata']['components'] as $component) {
            if ($component['type'] === 'BODY') {
                $templateBody = $component;
                break;
            }
        }
            if (!$templateBody) {
            return response()->json(['error' => 'No body content found in template'], 400);
        }
        // Map placeholders with received data
        $values = [
            "{{1}}" => $data['customer_name'] ?? 'N/A',
            "{{2}}" => $data['invoice_number'] ?? 'N/A',
            "{{3}}" => $data['company_name'] ?? 'N/A',
            "{{4}}" => $data['0'] ?? 'N/A', // Payments Received
            "{{5}}" => $data['1'] ?? 'N/A', // Total Recorded Payment
            "{{6}}" => $data['2'] ?? 'N/A', // Amount Due
            "{{7}}" => array_key_last($data), // Invoice URL
        ];
        ...
        $whatsappResponse = Http::post('https://mywhatschat.in/api/send/template', $payload);

        // Log response (for debugging)
        Log::info('WhatsApp Response:', $whatsappResponse->json());
