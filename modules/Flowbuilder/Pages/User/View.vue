<script setup lang="ts">
    ...
    const handleClickPublishBtn = async () => {
      try {
        const url = status.value == 'inactive' ? `/automation/flows/${props.uuid}/publish` : `/automation/flows/${props.uuid}/unpublish`;
    ...
    <div class="relative flex h-screen w-full flex-col">
      <header class="h-20 border-l border-b border-gray-200 bg-white px-4 py-3">
        <div class="flex h-full items-center justify-between">
          ...
          <div class="flex gap-x-3">
            ...
            <Button variant="outline" :class="status == 'active' ? 'bg-red-500 text-white' : 'bg-green-700 text-white'" size="sm" @click="handleClickPublishBtn"> {{ status == 'active' ? 'Unpublish' : 'Publish' }}</Button>
          </div>
        </div>
      </header>
      <main class="relative flex h-full w-full flex-1">
        ...
        <div class="relative h-full flex-1 overflow-hidden">
          <main-canvas :uuid="props.uuid" :flow="props.flow" @updateStatus="handleStatusUpdate"/>
        </div>
      </main>
    </div>
  </div>
